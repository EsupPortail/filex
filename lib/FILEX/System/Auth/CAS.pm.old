...
sub _parse_url {
	...
	if ($1 eq 'http') {
	    ...
	    $self->{'errors'} = sprintf "Unknown protocol '%s'\n", $1;
	    return undef;
	}
    }else {
	$self->{'errors'} = sprintf "Unable to parse URL '%s'\n", $url;
	return undef;
    }
...
sub validateST {
	...
	unless (open STORE, $self->{'pgtFile'}) {
	    $self->{'errors'} = sprintf "Unable to read %s\n", $self->{'pgtFile'};
	    return undef;
	}
...
sub storePGT {
    ...
    unless (open STORE, ">>$self->{'pgtFile'}") {
	$self->{'errors'} = sprintf "Unable to write to %s\n", $self->{'pgtFile'};
	return undef;
    }
    printf STORE "%s\t%s\n", $pgtIou, $pgtId;
    close STORE;
...
sub get_https2{
	...
	if (($trusted_ca_file && !(-r $trusted_ca_file)) ||  
	    ...
	    $self->{'errors'} = sprintf "error : incorrect access to cafile $trusted_ca_file or capath $trusted_ca_path\n";
	    return undef;
	}
	...
	unless ($ssl_socket) {
	    $self->{'errors'} = sprintf "error %s unable to connect https://%s:%s/\n",&IO::Socket::SSL::errstr,$host,$port;
	    return undef;
	}
